...
**Authoring a platform release note** (after jazzsequence's pantheon-docs-writer, MIT; site-specific schema and paths belong in the site profile, `site-profiles.md`):
- *Belongs:* new features and enhancements, known issues with workarounds, required reader action to avoid disruption, visible UI changes, roadmap items cleared for disclosure. *Does not belong:* marketing, organizational or branding news with no technical content, legal or compliance changes with no reader-facing effect.
- *Date it to the day the change goes live or is announced*, not the day you write it; where the filename carries the date and slug, the filename is the URL and is checked like one. The publish timestamp drives feed order, so it must be real.
- *Lead with what changed and why it matters* in the first sentence. Headings carry the message ("Introducing X for faster builds"), not just the noun ("X launched"). Sentence case even when the site's docs use title case; bullets for multiple changes; present tense ("X 4.3.0 is now available", not "has been released"); titles specific ("PHP 7.2, 7.3, and 8.0 are now end of sale", not "PHP version update").
- *Three body shapes:* **simple announcement** (what changed, why it matters, link to the doc) · **action required** (what changed → `## Action required`: what to do, by when, what happens otherwise → upgrade link; always tagged with the site's action-required category) · **version release** (linked release, one sentence on what is new, `## Key improvements` as bullets each linking its PR, `## How to upgrade`).
- *Always link the relevant doc* by name ("See the Turbo Boost documentation"), never "check the docs". Screenshots for UI changes. Plain Markdown body unless the profile says components render there.
